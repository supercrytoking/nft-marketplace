    ...
    return (
        <LazyLoad>
            <Link key={`${nft.contractAddress}-${nft.tokenId}`} href={`/${nft.contractAddress}/${nft.tokenId}`} >
                <a className="relative group relative bg-white bg-opacity-5 border border-white border-opacity-10 rounded overflow-hidden flex items-center justify-center" style={{ minHeight: '8em' }}>
                    ...
                    <img className='max-h-full min-w-full object-contain' src={imageUrl(nft.metadata.image)} alt={nft?.metdata?.name} />
                </a>
            </Link >
        </LazyLoad>
    )
